...
class BaseCoach:
    ...
    def restart_training(self):
        ...
        finetuning.inject_lora(self.G, r=128)
    ...
    def calc_loss_reid(self, ReIDmodel, generated_images, real_images, log_name, new_G, use_ball_holder, w_batch):
        loss = 0.0
        id_loss_val = l2_loss_val = loss_lpips = 10
        if hyperparameters.pt_l2_lambda > 0:
            l2_loss_val = l2_loss.l2_loss(generated_images, real_images)
            if self.use_wandb:
                wandb.log({f'MSE_loss_val_{log_name}': l2_loss_val.detach().cpu()}, step=global_config.training_step)
        ...
        id_loss_val = identity_loss.identity_loss(real_images, generated_images,ReIDmodel, sim_up=True)
        loss += id_loss_val * hyperparameters.pt_id_lambda
