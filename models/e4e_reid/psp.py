...
from configs.paths_config import stylegan2_ada_reid, ir_se50
...
class pSp(nn.Module):
    ...
    def __init__(self, opts):
        ...
        self.decoder = None #Generator(opts.stylegan_size, 512, 8, channel_multiplier=2)
    ...
    def load_weights(self):
        if self.opts.checkpoint_path is not None:
            ...
            with open(stylegan2_ada_reid, 'rb') as f:
                ckpt = load_network_pkl(f)
            self.decoder = ckpt['G_ema'].cuda().eval()
            #self.decoder.load_state_dict(get_keys(ckpt, 'decoder'), strict=True)
            self.__load_latent_avg(ckpt)
        else:
            print('Loading encoders weights from irse50!')
            encoder_ckpt = torch.load(ir_se50['ir_se50'])
    ...
    def __load_latent_avg(self, ckpt, repeat=None):
        if 'G_ema' in ckpt and hasattr(ckpt['G_ema'], 'w_avg'):
            self.latent_avg = ckpt['G_ema'].w_avg.to(self.opts.device)
            ...
            with torch.no_grad():
                z_samples = torch.randn(10000, 512).to(self.opts.device)
                w_samples = self.decoder.mapping(z_samples, c=None)
                if w_samples.ndim == 2:
                    # [10000, 512] → [10000, 1, 512] for broadcasting
                    w_samples = w_samples.unsqueeze(1)
                self.latent_avg = w_samples.mean(dim=0)
